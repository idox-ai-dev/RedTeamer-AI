...
function setRunId(ocSession: string, runId: string | null): void {
  const f = runFile(ocSession);
  try {
...
function readEvents(since?: string, attackRunId?: string) {
  ...
  const enriched = all.map((e) => {
    if (e.phase === "prompt") {
      if (e.attack_method) currentAttackMethod = e.attack_method;
      if (e.session_id) currentSessionId = e.session_id;
    }
  ...
  return enriched.filter((e) => {
    if (since && new Date(e.timestamp).getTime() < sinceMs) return false;
    if (attackRunId && e.attack_run_id !== attackRunId) return false;
    return true;
  });
}
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          const { stdout, stderr, code } = await new Promise((resolve) => {
            const child = spawn(
              "openclaw",
              ["agent", "--session-id", scenario_id, "--message", prompt, "--json"],
              { encoding: "utf-8" }
            );
            let out = "", err = "";
